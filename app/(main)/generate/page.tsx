            ...
            {generatedPacks.length > 0 ? (
              generatedPacks.map((pack) => (
                <div key={pack.id} className="bg-white rounded-xl border-2 border-black brutalist-shadow-sm overflow-hidden">
                  <div className="p-3 flex items-center justify-between bg-zinc-50 border-b border-zinc-100 cursor-pointer" onClick={() => togglePack(pack.id)}>
                  ...
                  {pack.isExpanded && (
                    <div className="p-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      {pack.icons.map((icon, index) => (
                        <div key={`${icon.png?.key || icon.svg?.key || index}-${index}`} className="bg-zinc-50 rounded-lg border border-zinc-200 overflow-hidden">
                          <div className="aspect-square p-2 flex items-center justify-center bg-white">
                            {icon.preview ? <img src={icon.preview} alt={icon.prompt} className="max-w-full max-h-full object-contain" /> : <div className="text-zinc-400 text-xs">Loading...</div>}
                          </div>
                          <div className="p-1.5 flex gap-1">
                            {icon.png?.key && <Button variant="outline" size="sm" className="h-6 text-xs border border-black rounded flex-1" onClick={() => handleDownloadPng(icon.png!.key, icon.prompt)}>PNG</Button>}
                            {icon.svg?.key && <Button variant="outline" size="sm" className="h-6 text-xs border border-black rounded flex-1" onClick={() => handleDownloadSvg(icon.svg!.key, icon.prompt)}>SVG</Button>}
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="p-2 border-t border-zinc-100">
                    <Button variant="ghost" size="sm" className="w-full text-zinc-400 hover:text-red-500" onClick={() => setGeneratedPacks((prev) => prev.filter((p) => p.id !== pack.id))}>
              ...
              <div className="text-center py-12 sm:py-16">
                ...
                <h1 className="text-xl sm:text-2xl font-bold text-zinc-900 mb-2">Create Your Icon</h1>
                <p className="text-zinc-500 text-sm sm:text-base max-w-md mx-auto">Describe the icon you want to create and let AI do the magic</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="w-full max-w-2xl mx-auto">
            <div className="bg-white rounded-xl sm:rounded-2xl border-2 border-black brutalist-shadow p-2 sm:p-3">
              <div className="p-2 sm:p-3">
                <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} onKeyDown={handleKeyDown} placeholder="Describe your icon... e.g., 'A shopping cart icon with a small bag inside'" className="w-full resize-none border-none outline-none text-sm placeholder:text-zinc-400 min-h-[60px] sm:min-h-[80px]" rows={2} disabled={isGenerating} />
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2 px-2 sm:px-3 pb-2 sm:pb-3">
                <div className="flex items-center gap-2">
                  <DropdownMenu>
                    <DropdownMenuTrigger className="h-9 gap-1.5 text-xs sm:text-sm brutalist-border-2 rounded-lg flex-shrink-0 inline-flex items-center justify-center border border-black bg-white px-3 py-2 text-sm font-medium transition-colors hover:bg-zinc-100 disabled:opacity-50 cursor-pointer" disabled={isGenerating}>
                    ...
                    <DropdownMenuContent align="start" className="w-48">
                      <div className="px-2 py-1.5">
                        ...
                        <div className="flex gap-1 flex-wrap">
                          {[4, 8, 12, 16].map((count) => (<Button key={count} variant={format.count === count ? "default" : "outline"} size="sm" className="h-8 text-xs" onClick={() => setFormat({ count })}>{count}</Button>))}
                        </div>
                      </div>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <DropdownMenu>
                    <DropdownMenuTrigger className="h-9 text-xs sm:text-sm text-zinc-500 inline-flex items-center justify-center transition-colors hover:bg-zinc-100 disabled:opacity-50 cursor-pointer" disabled={isGenerating}>
                    ...
                    <DropdownMenuContent align="start" className="w-56">
                      <div className="space-y-1 p-1">
                        {STYLES.map((style) => (<DropdownMenuItem key={style.id} className={`flex items-center justify-between cursor-pointer ${selectedStyle === style.id ? 'bg-zinc-100' : ''}`} onClick={() => setSelectedStyle(style.id)}><div><div className="text-sm">{style.name}</div><div className="text-xs text-zinc-500">{style.description}</div></div>{selectedStyle === style.id && <HiCheck className="h-4 w-4" />}</DropdownMenuItem>))}
                      </div>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                <Button className="bg-[#B9FF66] text-black border-2 border-black rounded-xl px-4 sm:px-6 h-9 text-sm font-semibold brutalist-shadow-sm hover:bg-[#a8ef55] w-full sm:w-auto disabled:opacity-50" onClick={handleGenerate} disabled={isGenerating || !prompt.trim()}>
                  {isGenerating ? <span className="animate-pulse">Generating...</span> : <><HiArrowTopRightOnSquare className="h-4 w-4 ml-1.5" />Generate</>}
                </Button>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 mt-3 sm:mt-4 justify-center">
              {["User profile avatar", "Settings gear", "Shopping cart", "Notification bell", "Message bubble"].map((suggestion) => (<QuickPromptButton key={suggestion} suggestion={suggestion} onClick={setPrompt} disabled={isGenerating} />))}
            </div>
          </div>
        </div>
      </div>
